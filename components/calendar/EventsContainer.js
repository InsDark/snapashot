...
const EventsContainer = ({ events, currentDate }) => {
  ...
  return (
    <View style={{ height: '100%', marginTop: 15 }}>
      <Text style={{ color: COLORS.darkBlue, fontSize: 25, fontWeight: 'bold', backgroundColor: COLORS.white, textAlign: 'center', width: '50%', borderBottomRightRadius: 10, borderTopRightRadius: 10, padding: 4 }}>{dayjs(currentDate).format("DD MMMM")}</Text>
      <FlatList
        data={eventsFilter}
        ...
        renderItem={({ item }) => (<EventItem key={item.time.start + item.time.end} item={item}/>)}
      >
      </FlatList>
